...
class Model:
    ...
    def setup_cnn(self) -> None:
        ...
        for i in range(num_layers):
            ...
            # Apply BatchNormalization after convolution
            conv_norm = tf.keras.layers.BatchNormalization()(conv, training=self.is_train)

            


            # Apply ReLU activation after BatchNormalization
        ...
        # Set the final output of CNN layers
        self.cnn_out_4d = pool
        print("CNN setup completed.")
    ...
    def setup_rnn(self) -> None:
        ...
        self.rnn_out_3d = tf.squeeze(tf.nn.atrous_conv2d(value=concat, filters=kernel, rate=1, padding='SAME'),
                                     axis=[2])
        print("RNN setup completed.")

    def setup_ctc(self) -> None:
        """Create CTC loss and decoder."""
        print("Setting up CTC layers...")
        # BxTxC -> TxBxC
    ...
    def setup_tf(self) -> Tuple[tf.compat.v1.Session, tf.compat.v1.train.Saver]:
        ...
        # Use tf.compat.v1.train.Saver() to avoid deprecated API warning
        saver = tf.compat.v1.train.Saver(max_to_keep=1)  # saver saves model to file
        model_dir = '../model/'
        latest_snapshot = tf.train.latest_checkpoint(model_dir)  # Check if there's a saved model

        # If the model must be restored, there must be a snapshot
    ...
    def to_sparse(self, texts: List[str]) -> Tuple[List[List[int]], List[int], List[int]]:
        ...
        for batchElement, text in enumerate(texts):
            # Convert to string of labels (i.e., class-ids)
    ...
    def train_batch(self, batch: Batch) -> float:
        ...
        print(f"Training batch {self.batches_trained + 1}...")
        ...
        print(f"Batch {self.batches_trained} completed with loss: {loss_val:.4f}")
        return loss_val
    ...
    def save(self) -> None:
        """Save model to file."""
        print(f"Saving model snapshot {self.snap_ID + 1}...")
        self.snap_ID += 1
        self.saver.save(self.sess, '../model/snapshot', global_step=self.snap_ID)
        print(f"Model snapshot {self.snap_ID} saved.")
...
if __name__ == "__main__":
    try:
        ...
        char_list = list("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
        model = Model(char_list=char_list, decoder_type=DecoderType.BestPath)
        print("Model initialized successfully.")

        # Simulate a training batch
        print("Creating dummy batch...")
        dummy_batch = Batch(imgs=np.random.rand(5, 128, 32), gt_texts=["hello", "world", "test", "data", "batch"])
        print("Dummy batch created.")

        print("Training the model...")
        loss = model.train_batch(dummy_batch)
        print(f"Training completed. Loss: {loss}")
