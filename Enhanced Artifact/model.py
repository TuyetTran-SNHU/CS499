...
IMG_SIZE = (256, 32)  # Example (height, width)
...
class Model:
    ...
    def ctc_loss(self, y_true, y_pred):
        ...
        # Debugging prints
        print(f"y_pred shape: {y_pred.shape}, y_pred example: {y_pred[0, :5, :5].numpy()}")  # Logs a small slice
        print(f"y_true shape: {y_true.shape}, y_true example: {y_true[0, :].numpy()}")
        print(f"Input lengths: {input_length.numpy()}")  # Convert tensor to NumPy for evaluation
        print(f"Label lengths: {label_length.numpy()}")
    ...
    def train(self, data_loader, preprocessor, epochs):
        ...
        debugged_first_batch = False  # Add a flag to control debugging
        ...
        for epoch in range(epochs):
            ...
            while data_loader.has_next():
                ...
                # Debugging: Inspect the first batch before training
                if not debugged_first_batch:
                    batch_imgs = np.array(processed_batch.imgs)
                    batch_labels = processed_batch.gt_texts
                    print(f"Batch images shape: {batch_imgs.shape}")
                    print(f"Batch labels: {batch_labels}")
                    debugged_first_batch = True  # Set flag to avoid repeating debugging
